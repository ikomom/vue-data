  ...
  methods: {
    updateChart () {
      ...
      const option = {
        ...
        tooltip: this.ishasprop(optionData.enableLabelGroup, {
          formatter: function(params) {  
              let res = params[0].name+'</br>';  
              let myseries = optionData.series; 
              let names = []
              for (let i in myseries) {  
                for(let j=0;j<=myseries[i].data.length;j++){ 
                  if(j+'月' == params[0].name){
                    if(names.includes(myseries[i].name)){
                      res+='&emsp;'+myseries[i].labelGroup+' : '+myseries[i].data[j-1]+'</br>';
                    }else {
                      names.push(myseries[i].name)
                      res+=params[i].marker + myseries[i].name+'</br>'+'&emsp;'+myseries[i].labelGroup+' : '+myseries[i].data[j-1]+'</br>';
                    }          
                  } 
                }         
              }  
