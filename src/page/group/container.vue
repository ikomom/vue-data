...
export default {
  ...
  computed: {
    ...
    styleName () {
      ...
      return Object.assign({
        ...
        if(this.contain.config.backgroundColor){
          return
        } else if (this.contain.config.backgroundImage) {
          return {
  ...
  mounted () {
    ...
    window.addEventListener('message',function(e){  
      if(e.origin=="http://localhost:1888"){
          console.log(e.data);
      }             
    })  
    */
  ...
  methods: {
      ...
      if (id) {
        ...
        getObj(id).then(res => {
          const callback = () => {
            ...
            // 新添-nav特殊字符
            this.contain.nav = JSON.parse(config.component.replace(/&gt;/g,'>').replace(/&amp;/g,'&')) || [];
    ...
    calcData () {
      ...
      /* 新添项配置数据 */
      if (!this.contain.config.interInfo) this.contain.config.interInfo = []
      if (!this.contain.config.interData) this.contain.config.interData = {}
      /* 新添项配置数据end */
    },
    ...
    /* 大屏数据刷新新添项 */
    dataRefresh () {
      let ids = ['1288008490089222145', '1247363602578780162']
      if(this.contain.config.interInfo.length > 0 && ids.includes(this.contain.id)){
        this.contain.config.interInfo.forEach(ele => {
          bAxios(ele, {}).then(res => {
