        ...
        <el-tabs class="tabs"
          ...
          <el-tab-pane name="1"
            ...
            <el-form label-width="120px"
              ...
              <!-- 新添项 -->
              <el-form-item label="加密类型" v-if="activeObj.dataType===1">
                <avue-radio v-model="activeObj.encrytType"
                            :dic="dicOption.encrytType"></avue-radio>
              </el-form-item>
              <el-form-item label="数据选择"
                            v-if="activeObj.dataType===2">
                <el-cascader
                    v-model="activeObj.data"
                    ref="dataCascader"
                    :options="interList"
                    @visible-change="onBlurAddressFun"
              ...
              <template v-if="activeObj.dataType===1">
                ...
                <!-- 新添项 -->
                <el-form-item label="传参类型" v-if="activeObj.dataMethod == 'post'">
                  <avue-radio v-model="activeObj.postType"
          ...
          <el-tab-pane name="2"
            ...
            <el-form label-width="120px"
              ...
              <!-- 新添项 -->
              <el-form-item label="刷全局">
                <avue-switch v-model="activeObj.child.allFresh"></avue-switch>
              </el-form-item>
              <!-- 新添项end -->
            </el-form>
          </el-tab-pane>
          ...
          <el-tab-pane name="5" v-else>
            ...
            <avue-crud :option="interOption"
                        :data="config.interInfo"
                        @row-save="rowSave"
                        @row-del="rowDel"
...
import { dicOption, interOption } from '@/option/config'
import init from '@/mixins/'
import { uuid, getDataType } from '@/utils/utils'
...
export default {
  ...
  computed: {
    ...
    interList () {
      let result = [];
      Object.keys(this.config.interData).forEach((item, index)=> {
        let parent = {
          'label': item,
          'value': this.config.interData[item]
        }
        if (getDataType(this.config.interData[item]) == 'Object') {
          parent.children = this.getChilds(this.config.interData[item]);
        }
        result.push(parent);
      })
      return result;
    ...
    activeKey () {
      ...
      } else if (['bar', 'line', 'pie', 'table','annular',
       'barline', 'bsankey', 'cumulate', 'gploygonal', 'gradual',
       'linebar', 'ploygonal'].includes(this.activeComponent.name)) {
        return this.activeOption.barColor
      }else {
        return this.config.interInfo
      }
      return []
    },
    /* 新添项end */
  ...
  methods: {
    /* 赋值数据新添 */
    onBlurAddressFun(bol, areaLabel){
      if(!bol){
        areaLabel = this.$refs['dataCascader'].inputValue;
        areaLabel = areaLabel.split(' / ');
        this.activeObj.assignment = areaLabel;
      }
    },
    getChilds (items){
      let childs = [];
      Object.keys(items).forEach((item, index)=> {
        ...
        if (items[item] && getDataType(items[item]) == 'Object') {
          parent.children = this.getChilds(items[item]);
        }
        childs.push(parent);
      })
      return childs;
    ...
    handleSetimg (val, type) {
      ...
      } else if (type === 'activeOption.image') { //新添项
        this.activeOption.image = val;
